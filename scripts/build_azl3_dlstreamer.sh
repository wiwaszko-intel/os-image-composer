...
build_azl3_dlstreamer_image() {
  echo "Building AZL3 DLStreamer Image. (using os-image-composer binary)"
  echo "Ensuring we're in the working directory before starting builds..."
  cd "$WORKING_DIR"
  echo "Current working directory: $(pwd)"
  
  set +e
  output=$(sudo -S ./os-image-composer build image-templates/azl3-x86_64-dlstreamer.yml 2>&1)
  build_exit_code=$?
  set -e
